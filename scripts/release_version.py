...
``sync --set X.Y.Z`` also sets CITATION.cff ``date-released`` to today and turns the changelog's ``## [Unreleased]`` section into
...
def sync(root: Path, new_version: str | None = None) -> str:
    ...
    if new_version is not None:
        ...
        updates[root / "CITATION.cff"] = set_date_released(
            replace_field(updates[root / "CITATION.cff"], r"^(version: *)[^\n]+$", version), date.today().isoformat()
        )
